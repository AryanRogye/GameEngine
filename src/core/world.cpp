...
Player* World::getPlayer() { return this->player; }
...
void World::updateAndRender()
{
     ...
     **/
    float oldX = this->getPlayer()->getX();
    float oldY = this->getPlayer()->getY();
...
{
    if(!this->keep_window_open) return;
    int breakableState = this->bf.checkBlockState(this->getPlayer()->getWorldHitbox(), this->mapData);
...
{
    if(!this->keep_window_open) return;
    int enterableState = this->bf.checkBlockState(this->getPlayer()->getWorldHitbox(), this->mapData);
